...
- **Attachments heal with tombstones.** The seed re-inserts missing `cohort_sections` attachments EXCEPT `(cohort_id, slug)` pairs in `seeded_section_removals`, which the admin bulk-save route syncs (seeded module absent from saved list → tombstone; re-added → tombstone cleared). The seed self-provisions the table with `CREATE TABLE IF NOT EXISTS`, so prod works without a manual schema sync. Admin deletions of seeded modules now survive restarts.
- **Reorder UI renumbers per level.** moveWithinLevel renumbers 1..n instead of swapping sortOrder values — duplicate sortOrders (from past re-seeding) made swaps no-op and rows got stuck.
